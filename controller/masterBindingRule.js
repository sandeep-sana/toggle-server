const Dao = require('../dao/masterBindingRule');
...
const fetchs = async (req, res) => {
  ...
  try {
    const masterBindingRules = await Dao.find(dbName, query, projection, options);
    if (!masterBindingRules) {
      return res.status(STATUS.INTERNAL_SERVER_ERROR).json({ message: MESSAGE.SERVER_ERROR });
    }
    return res.status(STATUS.OK).json({ masterBindingRules });
  } catch (error) {
    console.error(error);
...
const add = async (req, res) => {
  ...
  try {
    const master = await Dao.findOneAndUpdate(dbName, query, projection, options);
    if (!master) {
      return res.status(STATUS.INTERNAL_SERVER_ERROR).json({ message: MESSAGE.SERVER_ERROR });
    }
    return res.status(STATUS.OK).json({ master, message: MESSAGE.MASTER_BINDING_RULE.ADD_UPDATE });
  } catch (error) {
    console.error(error);
    res.status(STATUS.INTERNAL_SERVER_ERROR).json({ message: MESSAGE.SERVER_ERROR });
  }
